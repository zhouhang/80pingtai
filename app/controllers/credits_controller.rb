class CreditsController < ApplicationController
  ...
  def give_credit_to_other
    ...
    if status.nil?
      user = User.find(current_user.id)
      getCreditUser = User.where("login = ?", account)
      getCreditUser[0].increment!(:credit, money)
      user.decrement!(:credit, money)
      status = 1
      fundslog = Fundslog.new(:desc=>'给其他账号转款',:money=>money,:cur_money=>current_user.credit,:cur_commission=>current_user.commission,:staff_id=>current_user.id,:user_id=>current_user.id,:transaction_id=>0)
      fundslog.save
    end
